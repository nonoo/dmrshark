...
void command_process(char *input_buffer) {
	...
	if (strcmp(tok, "sms") == 0) {
		...
		smstxbuf_add(0, NULL, 0, d.sms.calltype, d.sms.dstid, DMR_DATA_TYPE_NORMAL_SMS, tok, 0, NULL);
		smstxbuf_add(0, NULL, 0, d.sms.calltype, d.sms.dstid, DMR_DATA_TYPE_MOTOROLA_TMS_SMS, tok, 0, NULL);
		return;
	}
