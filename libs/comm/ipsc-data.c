...
static void ipsc_data_handle_fragment_assembly_for_short_data_defined(ipscpacket_t *ipsc_packet, repeater_t *repeater) {
...
void ipsc_data_handle_34rate(struct ip *ip_packet, ipscpacket_t *ipsc_packet, dmrpacket_payload_bits_t *packet_payload_bits, repeater_t *repeater) {
	...
	if (repeater->slot[ipsc_packet->timeslot-1].data_packet_header.common.data_packet_format == DMRPACKET_DATA_HEADER_DPF_SHORT_DATA_DEFINED) { // Now we only care about short data packets.
		...
		ipsc_data_handle_fragment_assembly_for_short_data_defined(ipsc_packet, repeater);
	}
}
