...
static void rs_12_9_calc_error_evaluator_poly(rs_12_9_poly_t *error_locator_poly, rs_12_9_poly_t *syndrome, rs_12_9_poly_t *error_evaluator_poly) {
	uint8_t i;
...
static uint8_t rs_12_9_compute_discrepancy(rs_12_9_poly_t *error_locator_poly, rs_12_9_poly_t *syndrome, uint8_t L, uint8_t n) {
	uint8_t i;
	uint8_t sum = 0;
...
static void rs_12_9_calculate(rs_12_9_poly_t *syndrome, rs_12_9_poly_t *error_locator_poly, rs_12_9_poly_t *error_evaluator_poly) {
	uint8_t n;
	uint8_t L = 0;
	uint8_t L2;
	int8_t k;
...
static rs_12_9_roots_t *rs_12_9_find_roots(rs_12_9_poly_t *error_locator_poly) {
	...
	for (r = 1; r < 256; r++) {
		...
		for (k = 0; k < RS_12_9_CHECKSUMSIZE+1; k++)
			sum ^= rs_12_9_galois_multiplication(rs_12_9_galois_exp_table_get((k*r) % 255), error_locator_poly->data[k]);
...
void rs_12_9_calc_syndrome(rs_12_9_codeword_t *codeword, rs_12_9_poly_t *syndrome) {
	...
	for (j = 0; j < 3;  j++) {
		for (i = 0; i < sizeof(rs_12_9_codeword_t); i++)
			syndrome->data[j] = codeword->data[i] ^ rs_12_9_galois_multiplication(rs_12_9_galois_exp_table_get(j+1), syndrome->data[j]);
	}
}
	...
	*errors_found = roots->errors_num;
	...
	if (roots->errors_num > 0 && roots->errors_num <= RS_12_9_CHECKSUMSIZE) {
		...
		for (r = 0; r < roots->errors_num; r++) {
			...
			num = 0;
			for (j = 0; j < RS_12_9_POLY_MAXDEG; j++)
				num ^= rs_12_9_galois_multiplication(error_evaluator_poly.data[j], rs_12_9_galois_exp_table_get(((255-i)*j) % 255));
...
rs12_9_checksum_t *rs_12_9_calc_checksum(rs_12_9_codeword_t *codeword) {
	...
	static rs12_9_checksum_t rs12_9_checksum;
	uint8_t i;
	uint8_t feedback;

	rs12_9_checksum.bytes[0] = rs12_9_checksum.bytes[1] = rs12_9_checksum.bytes[2];

	for (i = 0; i < 9; i++) {
		feedback = codeword->data[i] ^ rs12_9_checksum.bytes[0];

		rs12_9_checksum.bytes[0] = rs12_9_checksum.bytes[1] ^ rs_12_9_galois_multiplication(genpoly[2], feedback);
		rs12_9_checksum.bytes[1] = rs12_9_checksum.bytes[2] ^ rs_12_9_galois_multiplication(genpoly[1], feedback);
		rs12_9_checksum.bytes[2] = rs_12_9_galois_multiplication(genpoly[0], feedback);
	}
	return &rs12_9_checksum;
}
